...
var channels_default = createRoute().get(async (c) => {
  ...
  const content = html`
      <div class="mx-auto max-w-2xl space-y-6">
        ...
        ${error ? html`<div class="alert alert-error text-sm">${decodeURIComponent(String(error))}</div>` : ""}
        ${connected ? html`<div class="alert alert-success text-sm">Channel connected successfully.</div>` : ""}
        ${placeholder(html`<p class="text-base-content/60">Loading…</p>`, renderChannelsPage(c, organizationId))}
      </div>
    `;
              ...
              <div class="flex flex-col gap-3 p-4">
                <div class="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    ${ch.targetType ? html`<p class="text-base-content/50 text-xs capitalize m-0">
                          ${ch.targetType} feed
                        </p>` : ""}
                    <span
                      ...
                      >${ch.status}</span
                    >
                    ${ch.lastError ? html`<p class="text-error mt-1 text-xs m-0">${ch.lastError}</p>` : ""}
                  </div>
