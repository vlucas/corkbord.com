// app/actions/start-channel-oauth-action.ts
...
var start_channel_oauth_action_default = createAction({
  name: "start-channel-oauth-action",
  schema
}).form((_c, { data, error }) => {
  const label = data?.provider === "x" ? "Connect X" : data?.targetType === "page" ? "Connect LinkedIn (page)" : "Connect LinkedIn (personal)";
  const btnClass = data?.provider === "x" ? "btn btn-primary btn-sm" : data?.targetType === "page" ? "btn btn-outline btn-sm" : "btn btn-secondary btn-sm";
  return html`
      <form>
        ${error ? html`<div class="alert alert-error text-sm">${error.message}</div>` : ""}
        <input type="hidden" name="provider" value="${data?.provider ?? ""}" />
        ${data?.targetType ? html`<input type="hidden" name="targetType" value="${data.targetType}" />` : ""}
...
export {
  start_channel_oauth_action_default as default
};
